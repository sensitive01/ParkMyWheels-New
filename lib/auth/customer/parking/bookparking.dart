...
class _ChooseParkingPageState extends State<ChooseParkingPage> {
  ...
  List<dynamic> charge = [];
  String? spaceid; // Add spaceid variable
  // ... existing code ...
...
// Helper function to round up to nearest rupee
  ...
  double calculateTotalWithTaxes(double payableAmount) {
    // Use original values for amount and handling fee
    double parkingCharge = payableAmount;
    double handlingFeeAmount = handlingFee;

    // GST base = amount + handling fee
    ...
    // Round off only the final payable amount
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (data is Map && data['spaceid'] != null) {
            spaceid = data['spaceid'].toString();
            print('Spaceid found: $spaceid');
          } else {
            spaceid = null;
          }
          ...
          if (data is List) {
            ...
            charge = List<Map<String, dynamic>>.from(data['charges']);
  ...
  void initState() {
    ...
    // Add listeners for real-time duplicate booking validation
    carController.addListener(_onVehicleOrDateChanged);
    dateTimeController.addListener(_onVehicleOrDateChanged);
    subscriptionDateTimeController.addListener(_onVehicleOrDateChanged);
    dateeTimeController.addListener(_onVehicleOrDateChanged);
  ...
  // Check duplicate booking after vendor and vehicle selection
  void _checkDuplicateAfterSelection() {
    if (selectedVendor != null && carController.value != null) {
      print('🔍 Checking duplicate booking after vendor and vehicle selection');
      print('   Vendor: ${selectedVendor?.vendorName}');
      print('   Vehicle: ${carController.value}');
      
      // Use selected date if available, otherwise use today's date for instant bookings
      DateTime dateToCheck = selectedDateTime ?? DateTime.now();
      String formattedDate = DateFormat("dd-MM-yyyy").format(dateToCheck);
      String? sts = _selectedOption == 'Subscription' ? 'subscription' : null;
      
      print('   Parking Date: $formattedDate');
      print('   Booking Type: ${_selectedOption ?? "Instant"}');
      
      _checkDuplicateBookingDebounced(carController.value!, formattedDate, sts);
    } else {
      setState(() {
        duplicateBookingError = null;
      });
      print('⏭️ Skipping duplicate check - Vendor or Vehicle not selected');
    }
  }

  // Real-time duplicate booking check
  void _onVehicleOrDateChanged() {
    if (carController.value != null && selectedDateTime != null) {
      String formattedDate = DateFormat("dd-MM-yyyy").format(selectedDateTime!);
      String? sts = _selectedOption == 'Subscription' ? 'subscription' : null;
      _checkDuplicateBookingDebounced(carController.value!, formattedDate, sts);
    } else {
      // If vendor and vehicle are selected but date is not, still check with today's date
      if (selectedVendor != null && carController.value != null) {
        _checkDuplicateAfterSelection();
      } else {
        setState(() {
          duplicateBookingError = null;
        });
  ...
  void _checkDuplicateBookingDebounced(String vehicleNumber, String parkingDate, String? sts) {
    _debounceTimer?.cancel();
    _debounceTimer = Timer(const Duration(milliseconds: 500), () {
      _checkDuplicateBooking(vehicleNumber, parkingDate, sts);
    });
  }

  // Check for duplicate booking
  Future<void> _checkDuplicateBooking(String vehicleNumber, String parkingDate, String? sts) async {
    if (vehicleNumber.isEmpty || parkingDate.isEmpty) {
      setState(() {
        duplicateBookingError = null;
      });
      return;
    ...
    print('🔍 Starting duplicate booking check...');
    print('   Vehicle Number: $vehicleNumber');
    print('   Parking Date: $parkingDate');
    print('   Subscription Type: ${sts ?? "none"}');
    ...
    try {
      ...
      final uri = Uri.parse('${ApiConfig.baseUrl}vendor/checkduplicatebooking')
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          if (data['isDuplicate'] == true) {
            duplicateBookingError = data['message'] ?? 'This vehicle is already booked for this date';
            print('❌ DUPLICATE BOOKING DETECTED!');
            print('   Error Message: $duplicateBookingError');
            print('   Vehicle: $vehicleNumber');
            print('   Date: $parkingDate');
          } else {
            duplicateBookingError = null;
            print('✅ No duplicate booking found - Vehicle is available');
          }
  ...
  void dispose() {
    ...
    dateTimeController.removeListener(_onVehicleOrDateChanged);
    subscriptionDateTimeController.removeListener(_onVehicleOrDateChanged);
    dateeTimeController.removeListener(_onVehicleOrDateChanged);
    super.dispose();
  }
  Future<void> _payNow() async {
    // Check for duplicate booking error first
    if (duplicateBookingError != null && duplicateBookingError!.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(duplicateBookingError!)),
      );
      return;
    }
    ...
    if (_selectedOption == 'Subscription') {
      DateTime endDate = selectedDateTime!.add(const Duration(days: 30));
      subscriptionEndDate = DateFormat("dd-MM-yyyy hh:mm a").format(endDate);
    }
    ...
    // Check if spaceid exists - if yes, skip payment and create booking directly
    if (_selectedOption == 'Subscription' && spaceid != null && spaceid!.isNotEmpty) {
      await _createSubscriptionBookingDirectly(formattedDate, formattedTime, amount, now, subscriptionEndDate);
      return;
    }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => monthlysub(
          ...
          subsctiptionenddate: subscriptionEndDate, // Pass calculated end date
        ),
      ),
    );
  }

  Future<void> _createSubscriptionBookingDirectly(
    String formattedDate,
    String formattedTime,
    String amount,
    DateTime now,
    String subscriptionEndDate, // Add parameter
  ) async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      var data = {
        ...
        'vehicleType': selectedCarType ?? '',
        'parkingTime': formattedTime,
        'parkedDate': formattedDate, // Set parked date
        'parkedTime': formattedTime, // Set parked time
        'approvedDate': formattedDate, // Set approved date
        'approvedTime': formattedTime, // Set approved time
        'bookingTime': DateFormat("hh:mm a").format(now),
        'cancelledStatus': "",
        'bookType': isHourly ? 'Hourly' : '24 Hours',
        'invoice': '', // No payment, so no invoice
        'subsctiptionenddate': subscriptionEndDate, // Include end date
      };

      print('Creating booking directly (spaceid present): ${jsonEncode(data)}');
      ...
      if (response.statusCode == 200) {
        ...
        final String bookingId = responseData['bookingId'] ?? responseData['_id'] ?? '';
        ...
        String schedule = '$formattedDate $formattedTime';

        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => bookticket(
              bookingtype: isHourly ? 'Hourly' : '24 Hours',
              otp: otp,
              sts: 'Subscription',
              bookedid: bookingId,
              vehiclenumber: carController.value ?? '',
        ...
        if (errorData['error'] == 'DUPLICATE_BOOKING') {
          setState(() {
            duplicateBookingError = errorData['message'] ?? 'This vehicle is already booked for this date';
          });
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(duplicateBookingError!)),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(errorData['message'] ?? 'Failed to create booking')),
          );
        }
      } else {
        final errorData = jsonDecode(response.body);
  ...
  Future<void> _registerParking() async {
    ...
    try {
      ...
      // If subscription with spaceid, set parked and approved dates/times
      if (_selectedOption == 'Subscription' && spaceid != null && spaceid!.isNotEmpty) {
        data['parkedDate'] = formattedDate;
        data['parkedTime'] = formattedTime;
        data['approvedDate'] = formattedDate;
        data['approvedTime'] = formattedTime;
      }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: SingleChildScrollView(
                  ...
                  CustomDropdown<String?>.search(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        ...
                        duplicateBookingError = null; // Clear duplicate error when vendor changes
                        ...
                        // Check for duplicate booking after vendor selection
                        if (selectedVendor != null && carController.value != null) {
                          print('🔄 Vendor selected - Checking duplicate booking...');
                          _checkDuplicateAfterSelection();
                        }
                      });
                    },
                        ...
                        onChanged: (String? value) {
                          setState(() {
                            ...
                            if (value != null) {
                              ...
                              charge = [];  // Clear subscription charges
                              availableSlots = 0; // Reset available slots
                              duplicateBookingError = null; // Clear duplicate error when vehicle changes
                              if (selectedVendor != null) {
                                checkBusinessHours(selectedVendor!.id).then((_) {
                                  checkIfVendorIsOpen();
                                });
                              ...
                              // Check for duplicate booking after vehicle selection
                              if (selectedVendor != null && carController.value != null) {
                                print('🔄 Vehicle selected - Checking duplicate booking...');
                                _checkDuplicateAfterSelection();
                              }
                            } else {
            ...
            //
              ...
              if (_selectedOption != 'Subscription') ...[
                ...
                const SizedBox(height: 20),
                if (availableSlots == 0 || charges.isEmpty || !isVendorOpen || (duplicateBookingError != null && duplicateBookingError!.isNotEmpty)) ...[
                  // Show a message when no slots are available, no charges are found, vendor is closed, or duplicate booking
                  Builder(
                    builder: (context) {
                      String errorMessage = !isVendorOpen
                          ? closedReason
                          : duplicateBookingError != null && duplicateBookingError!.isNotEmpty
                      ...
                      if (duplicateBookingError != null && duplicateBookingError!.isNotEmpty) {
                        print('🚨 DUPLICATE BOOKING ERROR DISPLAYED:');
                        print('   Error Message: $duplicateBookingError');
                        print('   Vendor: ${selectedVendor?.vendorName}');
                        print('   Vehicle: ${carController.value}');
                        print('   Date: ${selectedDateTime != null ? DateFormat("dd-MM-yyyy").format(selectedDateTime!) : "Not selected"}');
                      }
              ...
              height: 40,
              child: ElevatedButton(
              onPressed: (_isLoading || (duplicateBookingError != null && duplicateBookingError!.isNotEmpty))
              ? null
          : (_selectedOption == 'Subscription' && (spaceid == null || spaceid!.isEmpty)
              ? _payNow
          ...
          : Align(
              child: Text(
              (_selectedOption == 'Subscription' && (spaceid == null || spaceid!.isEmpty)) ? 'Pay Now' : 'Book Now', // 👈 choose text
              style: const TextStyle(
              color: Colors.white,
              fontWeight: FontWeight.bold,
              ),
