...
class _SummaryReportScreenState extends State<SummaryReportScreen> with TickerProviderStateMixin {
  ...
  Future<void> _fetchReports() async {
    ...
    final url = Uri.parse('${ApiConfig.baseUrl}vendor/getreports/${widget.vendorid}');
    try {
        ...
        if (reports.isNotEmpty) {
          ...
          try {
            ...
            setState(() {
              ...
              _startDateController.text = DateFormat('dd-MM-yyyy hh:mm a').format(_selectedStartDate!);
              _endDateController.text = DateFormat('dd-MM-yyyy hh:mm a').format(_selectedEndDate!);
            });
          } catch (_) {}
        }

        setState(() { _pastReports = reports; });
      }
    } catch (_) {
    } finally {
  ...
  Future<void> _fetchBookings() async {
    ...
    try {
      final start = DateFormat('dd-MM-yyyy').format(_selectedStartDate ?? _defaultStartDate);
      final end = DateFormat('dd-MM-yyyy').format(_selectedEndDate ?? _defaultEndDate);
      final url = Uri.parse(
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: SfDateRangePicker(
              ...
              rangeSelectionColor: ColorUtils.primarycolor().withAlpha(77), // ~30%
              todayHighlightColor: ColorUtils.primarycolor().withAlpha(26), // ~10%
  ...
  List<Bookingdata> _filteredBookings() {
    ...
    return _bookings.where((b) {
      ...
      final exitDateTime = (b.exitvehicledate.trim().isNotEmpty && b.exitvehicletime.trim().isNotEmpty) 
          ? _combineDateTime(b.exitvehicledate, b.exitvehicletime) 
          : null;

      bool isBookedInRange = false;
      if (bookingDateTime != null) {
        isBookedInRange = (bookingDateTime.isAtSameMomentAs(start) || bookingDateTime.isAfter(start)) &&
                          (bookingDateTime.isAtSameMomentAs(end) || bookingDateTime.isBefore(end));
      }

      bool isExitedInRange = false;
      if (exitDateTime != null) {
        isExitedInRange = (exitDateTime.isAtSameMomentAs(start) || exitDateTime.isAfter(start)) &&
  ...
  double _amountForBooking(Bookingdata b) {
    ...
    final isPassOrSub = isSubscriptionSts(sts) || RegExp(r'^\d+(?:hr|h)$', caseSensitive: false).hasMatch(sts);
    
    final valetAmount = isPassOrSub ? 0.0 : (double.tryParse(b.valetCharge) ?? 0.0);
  ...
  _SummaryTotals _computeTotals(List<Bookingdata> list) {
    ...
    for (final b in list) {
      ...
      } else if (pm.contains('online') || pm.contains('upi') || pm.contains('qr')) {
  ...
  Future<void> _saveReportToBackend({
    ...
    try {
      ...
      print('[SummaryReport.saveReportToBackend] ERROR POST ${ApiConfig.baseUrl}vendor/savereport -> $e');
  ...
  Future<void> _printReport() async {
    ...
    if (empId.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Employee ID is required to generate a report')),
      );
      return;
    }
    ...
    final exitCount = list.where((b) => b.exitvehicledate.trim().isNotEmpty).length;
    ...
    final printedStartDate = _isFirstReport 
        ? DateTime(startDate.year, startDate.month, startDate.day, 0, 0, 0)
        : startDate;
        
    final startLabel = DateFormat('dd-MM-yyyy hh:mm a').format(printedStartDate);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_isFirstReport)
            IconButton(
              icon: Icon(Icons.calendar_month, color: ColorUtils.primarycolor()),
                    ...
                    child: Column(
                      children: [
                        Row(
                          ...
                          children: [
                            Text(_startDateController.text, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold)),
                            const SizedBox(width: 8),
                            Text('to', style: GoogleFonts.poppins(fontSize: 12)),
                            const SizedBox(width: 8),
                            Text(_endDateController.text, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold)),
                          ],
                        ),
                            ...
                            child: Padding(
                              ...
                              child: SingleChildScrollView(
                                child: Column(
                                  ...
                                  children: [
                                    _SummaryRow(label: 'Hourly', count: totals.hourlyCount, amount: totals.hourlyAmount),
                                    _SummaryRow(label: '12 Hours Pass', count: totals.pass12Count, amount: totals.pass12Amount),
                                    _SummaryRow(label: '24 Hours Pass', count: totals.pass24Count, amount: totals.pass24Amount),
                                    _SummaryRow(label: '48 Hours Pass', count: totals.pass48Count, amount: totals.pass48Amount),
                                    _SummaryRow(label: '72 Hours Pass', count: totals.pass72Count, amount: totals.pass72Amount),
                                    _SummaryRow(label: 'Weekly Pass (7 Days)', count: totals.weeklyCount, amount: totals.weeklyAmount),
                                    _SummaryRow(label: '15 Days Pass', count: totals.pass15Count, amount: totals.pass15Amount),
                                    _SummaryRow(label: 'Monthly Pass (30 Days)', count: totals.monthlyCount, amount: totals.monthlyAmount),
                                    const Divider(),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        Text('Cash Total:', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
                                        Text('₹${totals.cashTotal.toStringAsFixed(2)}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.green)),
                                      ],
                                    ),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        Text('Online Total:', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
                                        Text('₹${totals.onlineTotal.toStringAsFixed(2)}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.blue)),
                                      ],
                                    ),
                                    const Divider(),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        Text('Total Amount:', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16)),
                                        Text('₹${(totals.cashTotal + totals.onlineTotal).toStringAsFixed(2)}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 16)),
                                      ],
                                    ),
                                    const SizedBox(height: 10),
                                    Center(
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: Text('Generate & Print Report', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: ColorUtils.primarycolor(),
                              foregroundColor: Colors.white,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('No past reports found', style: GoogleFonts.poppins(color: Colors.grey)),
          ],
        ),
      );
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.poppins(fontSize: 12, color: Colors.blue[700], fontWeight: FontWeight.bold),
                    ),
                    Text(
            ...
            _buildInfoRow('From:', report['fromdate_time'] ?? 'N/A'),
            _buildInfoRow('To:', report['todate_time'] ?? 'N/A'),
            const SizedBox(height: 4),
            Row(
              children: [
                Expanded(child: _buildInfoRow('Entries:', '${report['entry'] ?? 0}', isBold: true)),
                const SizedBox(width: 20),
                Expanded(child: _buildInfoRow('Exits:', '${report['exit'] ?? 0}', isBold: true)),
              ],
            ),
            const SizedBox(height: 10),
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text('Re-print Report', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold)),
                style: ElevatedButton.styleFrom(
  ...
  Future<void> _reprintReport(dynamic report) async {
    try {
      ...
      if (_bookings.isNotEmpty) {
        vendorName = _bookings.first.vendorname.trim().isNotEmpty ? _bookings.first.vendorname : 'Parking Location';
      }
      ...
      await UniversalPrintHelper.printSummaryReport(
        ...
        fromTime: fromParts.length > 1 ? fromParts.sublist(1).join(' ') : '00:00',
        ...
        hourlyCount: int.tryParse(report['hourlycount']?.toString() ?? '0') ?? 0,
        hourlyAmount: double.tryParse(report['hourlyamount']?.toString() ?? '0') ?? 0.0,
        pass12Count: int.tryParse(report['12hrsveh']?.toString() ?? '0') ?? 0,
        pass12Amount: double.tryParse(report['12hrsvehamt']?.toString() ?? '0') ?? 0.0,
        pass24Count: int.tryParse(report['24hrsveh']?.toString() ?? '0') ?? 0,
        pass24Amount: double.tryParse(report['24hrsvehamt']?.toString() ?? '0') ?? 0.0,
        pass48Count: int.tryParse(report['48hrsveh']?.toString() ?? '0') ?? 0,
        pass48Amount: double.tryParse(report['48hrsvehamt']?.toString() ?? '0') ?? 0.0,
        pass72Count: int.tryParse(report['72hrsveh']?.toString() ?? '0') ?? 0,
        pass72Amount: double.tryParse(report['72hrsvehamt']?.toString() ?? '0') ?? 0.0,
        weeklyCount: int.tryParse(report['7daysveh']?.toString() ?? '0') ?? 0,
        weeklyAmount: double.tryParse(report['7daysvehamt']?.toString() ?? '0') ?? 0.0,
        pass15Count: int.tryParse(report['15daysveh']?.toString() ?? '0') ?? 0,
        pass15Amount: double.tryParse(report['15daysvehamt']?.toString() ?? '0') ?? 0.0,
        monthlyCount: int.tryParse(report['30daysveh']?.toString() ?? '0') ?? 0,
        monthlyAmount: double.tryParse(report['30daysvehamt']?.toString() ?? '0') ?? 0.0,
        cashTotal: double.tryParse(report['cash']?.toString() ?? '0') ?? 0.0,
        onlineTotal: double.tryParse(report['online']?.toString() ?? '0') ?? 0.0,
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
...
class _SummaryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: Text(label, style: GoogleFonts.poppins(fontSize: 13, color: Colors.black))),
          SizedBox(
            width: 45,
            child: Text(
              count.toString(),
