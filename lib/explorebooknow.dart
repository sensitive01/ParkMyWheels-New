...
class _ExploreBookState extends State<ExploreBook> {
  ...
  Future<void> fetchmonth(String selectedCarType) async {
    if (!mounted) return;
    setState(() {
      _activeRequests++; _isLoading = _activeRequests > 0;
    });
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        if (_activeRequests > 0) _activeRequests--; _isLoading = _activeRequests > 0;
      });
    }
  }
  Future<void> fetchChargesData(String selectedCarType) async {
    if (!mounted) return;
    setState(() { _activeRequests++; _isLoading = _activeRequests > 0; });
    try {
      ...
      if (!mounted) return;
      setState(() { if (_activeRequests > 0) _activeRequests--; _isLoading = _activeRequests > 0; });
    }
  }
  ...
  void _checkDuplicateAfterSelection() {
    if (widget.vendorId != null && carController.value != null) {
      print('🔍 Checking duplicate booking after vendor and vehicle selection');
      print('   Vendor: ${widget.vendorName}');
            ...
            child: BottomPicker.dateTime(
              ...
              onSubmit: (dateTime) {
                ...
                setState(() {
                  ...
                  // Check for duplicate booking when date changes
                  _onVehicleOrDateChanged();
                });
              },
  ...
  Widget build(BuildContext context) {
        ...
        child: Padding(
         ...
         Container(
              ...
              if (selectedVendor != widget.vendorName && selectedCarType != null) ...[
                if (!_isLoading && (availableSlots == 0|| charges.isEmpty)) ...[
                  const SizedBox(height: 15),
                  Text(
    ...
    if (_selectedOption != 'Subscription') ...[
              ...
              const SizedBox(height: 20),
              if (!_isLoading && (availableSlots == 0 || charges.isEmpty || !isVendorOpen || (duplicateBookingError != null && duplicateBookingError!.isNotEmpty))) ...[
...
// Helper function to round up to nearest rupee
  ...
  Future<void> fetchmonth(String vendorId, String selectedCarType) async {
    setState(() {
      _activeRequests++; _isLoading = _activeRequests > 0; // Start loading
    });
    ...
    try {
      ...
      setState(() {
        if (_activeRequests > 0) _activeRequests--; _isLoading = _activeRequests > 0; // Stop loading
      });
    }
  }
